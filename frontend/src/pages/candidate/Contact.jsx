  ...
  return (
    <div className="space-y-4">
      <div className="card flex items-center gap-3">
        ...
        <div>
          <p className="font-bold text-slate-800">فريق التوظيف</p>
          <p className="text-xs text-emerald-500">متصل — يرد عادةً خلال يوم عمل</p>
        </div>
      </div>
          ...
          {messages.map((m) => (
            <div key={m.id} className={`flex ${m.sender === 'candidate' ? 'justify-start' : 'justify-end'}`}>
              <div className={`max-w-[75%] rounded-2xl px-4 py-2 ${m.sender === 'candidate' ? 'bg-blue-600 text-white rounded-tr-sm' : 'bg-slate-100 text-slate-700 rounded-tl-sm'}`}>
                ...
                <p className={`text-[10px] mt-1 ${m.sender === 'candidate' ? 'text-white/70' : 'text-slate-400'}`}>{m.sender === 'candidate' ? 'أنت' : 'فريق التوظيف'} · {formatDate(m.created_at)}</p>
              </div>
            </div>
          ))}
          {messages.length === 0 && <p className="text-sm text-slate-400 text-center py-6">ابدأ المحادثة مع فريق التوظيف</p>}
        </div>
        <form onSubmit={submit} className="flex items-center gap-2 border-t border-slate-100 pt-3">
          <input value={text} onChange={(e) => setText(e.target.value)} placeholder="اكتب رسالتك…" className="input-field flex-1" />
