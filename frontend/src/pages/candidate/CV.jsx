...
function UploadModal({ current, onClose }) {
  ...
  const m = useMutation((cv_file_name) => candidateApi.updateProfile({ cv_file_name }), {
    onSuccess: () => { toast.success('تم تحديث السيرة الذاتية'); qc.invalidateQueries('candidate-profile'); onClose() },
    onError: () => toast.error('فشلت العملية'),
  })
  return (
    <Modal open onClose={onClose} title="تحديث السيرة الذاتية">
      <div className="space-y-4">
        <div className="rounded-xl border border-dashed border-slate-200 p-8 text-center">
          <UploadCloud className="w-10 h-10 text-slate-300 mx-auto" />
          <p className="text-sm text-slate-500 mt-2">اسحب الملف هنا أو أدخل اسمه (PDF/DOCX)</p>
        </div>
        <Field label="اسم ملف السيرة الذاتية"><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="cv_name.pdf" /></Field>
        <div className="flex justify-end gap-3 pt-2">
          <Button variant="secondary" onClick={onClose}>إلغاء</Button>
          <Button onClick={() => name && m.mutate(name)} loading={m.isLoading} disabled={!name}>حفظ</Button>
        </div>
      </div>
    </Modal>
  )
    ...
    <div className="space-y-6">
      <div className="card">
        <div className="flex items-center justify-between">
          <h3 className="font-bold text-slate-800">اكتمال الملف</h3>
        ...
        <p className="text-xs text-slate-400 mt-2">أكمل ملفك وارفع سيرتك الذاتية لزيادة فرص القبول.</p>
      </div>

      <div className="card">
        <h3 className="font-bold text-slate-800 mb-4">السيرة الذاتية</h3>
        {cv ? (
          <div className="flex items-center gap-4 rounded-xl border border-slate-100 p-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-emerald-600 flex items-center gap-1"><CheckCircle2 className="w-3.5 h-3.5" /> تم الرفع</p>
            </div>
            <button className="text-slate-400 hover:text-blue-600" title="تنزيل"><Download className="w-5 h-5" /></button>
            <Button variant="secondary" onClick={() => setShow(true)}><RefreshCw className="w-4 h-4" /> استبدال</Button>
          </div>
        ) : (
          <div className="rounded-xl border border-dashed border-slate-200 p-10 text-center">
            <UploadCloud className="w-12 h-12 text-slate-300 mx-auto" />
            <p className="text-sm text-slate-500 mt-3">لم ترفع سيرتك الذاتية بعد</p>
            <div className="mt-4"><Button onClick={() => setShow(true)}><UploadCloud className="w-4 h-4" /> رفع السيرة الذاتية</Button></div>
          </div>
        )}
      </div>
