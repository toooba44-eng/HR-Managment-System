...
function FillModal({ form, onClose }) {
  ...
  const m = useMutation(() => candidateApi.submitForm(form.id, { response }), {
    onSuccess: () => { toast.success('تم إرسال النموذج'); qc.invalidateQueries('cand-forms'); onClose() },
  ...
  return (
    <Modal open onClose={onClose} title={form.title}>
      <div className="space-y-4">
        ...
        <Field label="بياناتك / إقرارك"><Input value={response} onChange={(e) => setResponse(e.target.value)} placeholder="اكتب ردك هنا" /></Field>
        <label className="flex items-center gap-2 text-sm text-slate-600">
          <input type="checkbox" required id="ack" /> أقرّ بصحة البيانات المدخلة
        </label>
        <div className="flex justify-end gap-3 pt-2">
          <Button variant="secondary" onClick={onClose}>إلغاء</Button>
          <Button onClick={() => m.mutate()} loading={m.isLoading}>إرسال</Button>
        </div>
      </div>
    </Modal>
  )
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        <StatCard icon={ClipboardList} label="إجمالي النماذج" value={s.total ?? 0} tone="blue" />
        <StatCard icon={CheckCircle2} label="مكتملة" value={`${s.completed ?? 0}/${s.total ?? 0}`} tone="green" />
      </div>

      {items.length === 0 ? (
        <div className="card"><EmptyState icon={FileSignature} title="لا توجد نماذج مطلوبة" /></div>
      ) : (
          ...
          {items.map((f) => (
            <div key={f.id} className="card flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-400">{f.description}{f.submitted_at ? ` · أُرسل ${formatDate(f.submitted_at)}` : ''}</p>
              </div>
              <Badge status={f.status} />
              {f.status !== 'مكتمل' && (
                <button onClick={() => setFill(f)} className="text-sm px-3 py-2 rounded-xl bg-blue-50 text-blue-600 hover:bg-blue-100 flex items-center gap-1"><PenLine className="w-4 h-4" /> تعبئة</button>
              )}
            </div>
          ))}
        </div>
