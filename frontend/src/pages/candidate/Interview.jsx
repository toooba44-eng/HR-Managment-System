    ...
    <div className="space-y-6">
      {upcoming[0] && (
        <div className="card bg-gradient-to-br from-blue-600 to-violet-700 text-white">
          <p className="text-xs text-white/70">{t('مقابلتك القادمة')}</p>
          ...
          <div className="flex flex-wrap items-center gap-4 mt-3 text-sm text-white/90">
            ...
            <span className="badge bg-white/20 text-white">{t(upcoming[0].mode)}</span>
            <span className="badge bg-white/20 text-white">{t(upcoming[0].stage)}</span>
          </div>
          {upcoming[0].meeting_link && (
            <a href={upcoming[0].meeting_link} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 mt-4 px-4 py-2 rounded-xl bg-white text-blue-700 text-sm font-medium hover:bg-white/90">
              <Video className="w-4 h-4" /> {t('الانضمام للمقابلة')}
            </a>
          )}
        </div>
      )}
      ...
      {items.length === 0 ? (
        <div className="card"><EmptyState icon={CalendarDays} title={t('لا توجد مقابلات مجدولة')} /></div>
      ) : (
            ...
            return (
              <div key={iv.id} className="card flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400">{t(iv.stage)} · {t(iv.mode)}{iv.location ? ` · ${iv.location}` : ''}</p>
                ...
                <div className="text-left shrink-0">
                  ...
                  {iv.status === 'مجدولة' && iv.meeting_link && (
                    <a href={iv.meeting_link} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs text-blue-600 hover:underline mt-1"><ExternalLink className="w-3 h-3" /> {t('رابط')}</a>
                  )}
                </div>
              </div>
            )
