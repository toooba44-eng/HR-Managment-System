...
export default function TalentPool() {
  ...
  const m = useMutation((join) => candidateApi.setTalentPool(join), {
    onSuccess: (_, join) => { toast.success(join ? 'انضممت لقاعدة المواهب' : 'تم إلغاء الانضمام'); qc.invalidateQueries('candidate-profile') },
  ...
  return (
    <div className="space-y-6">
      <div className={`card text-center ${joined ? 'bg-gradient-to-br from-blue-600 to-violet-700 text-white' : ''}`}>
        ...
        <h2 className="text-xl font-bold mt-4">{joined ? 'أنت عضو في قاعدة المواهب' : 'انضم إلى قاعدة المواهب'}</h2>
        <p className={`text-sm mt-2 ${joined ? 'text-white/80' : 'text-slate-500'}`}>
          {joined ? 'ملفك ظاهر لمسؤولي التوظيف وسيتم إشعارك بالفرص المناسبة.' : 'احفظ ملفك ليصل إلى مسؤولي التوظيف عند توفّر فرص مناسبة.'}
        </p>
        <div className="mt-5">
          {joined ? (
            <button onClick={() => m.mutate(false)} disabled={m.isLoading} className="px-5 py-2 rounded-xl bg-white/20 hover:bg-white/30 text-white text-sm font-medium">إلغاء الانضمام</button>
          ) : (
            <Button onClick={() => m.mutate(true)} loading={m.isLoading}><Star className="w-4 h-4" /> الانضمام الآن</Button>
          )}
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {BENEFITS.map((b) => (
          <div key={b.title} className="card">
            ...
            <h3 className="font-bold text-slate-800 mt-3">{b.title}</h3>
      ...
      {joined && (
        <div className="card flex items-center gap-3">
          ...
          <p className="text-sm text-slate-600">حالتك: <span className="font-medium text-slate-800">عضو نشط في قاعدة المواهب</span></p>
        </div>
      )}
    </div>
  )
