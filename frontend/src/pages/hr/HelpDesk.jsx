...
function NewTicketForm({ open, onClose }) {
  ...
  const m = useMutation((d) => helpdeskApi.create(d), {
    onSuccess: () => { toast.success(t('تم إنشاء التذكرة')); qc.invalidateQueries('helpdesk'); onClose(); setForm({ category: 'استفسار عام', subject: '', description: '', priority: 'متوسطة' }) },
    onError: (e) => toast.error(e.response?.data?.error || t('فشل الإرسال')),
  })
  const set = (k) => (e) => setForm((f) => ({ ...f, [k]: e.target.value }))
  return (
    <Modal open={open} onClose={onClose} title={t('تذكرة دعم جديدة')}>
      <form onSubmit={(e) => { e.preventDefault(); m.mutate(form) }} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <Field label={t('التصنيف')}>
            <Select value={form.category} onChange={set('category')}>
              {['استفسار عام', 'رواتب ومزايا', 'إجازات وحضور', 'مشكلة تقنية', 'شكوى', 'أخرى'].map((c) => <option key={c} value={c}>{t(c)}</option>)}
            </Select>
          </Field>
          <Field label={t('الأولوية')}>
            <Select value={form.priority} onChange={set('priority')}>
              {['منخفضة', 'متوسطة', 'عالية', 'عاجلة'].map((p) => <option key={p} value={p}>{t(p)}</option>)}
            </Select>
          </Field>
        </div>
...
function TicketModal({ ticketId, onClose, canManage }) {
  ...
  const update = useMutation((d) => helpdeskApi.update(ticketId, d), { onSuccess: () => { toast.success(t('تم التحديث')); invalidate() }, onError: () => toast.error(t('فشل التحديث')) })
  const sendReply = useMutation(() => helpdeskApi.reply(ticketId, reply), {
    onSuccess: () => { setReply(''); invalidate() },
    onError: (e) => toast.error(e.response?.data?.error || t('فشل الرد')),
  })

  return (
    <Modal open={!!ticketId} onClose={onClose} title={t('تذكرة الدعم')} size="lg">
      {isLoading || !data ? (
        ...
        <div className="space-y-4">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              <span className={`badge ${PRIORITY_TONE[data.priority]}`}>{t(data.priority)}</span>
              <Badge status={data.status}>{t(data.status)}</Badge>
            </div>
          </div>
          <span className="badge bg-slate-100 text-slate-600 w-fit">{t(data.category)}</span>
            ...
            <div className="grid grid-cols-2 gap-3 pt-3 border-t border-slate-100">
              <Field label={t('الحالة')}>
                <Select value={data.status} onChange={(e) => update.mutate({ status: e.target.value })}>
                  {['مفتوحة', 'قيد المعالجة', 'بانتظار الموظف', 'مغلقة'].map((s) => <option key={s} value={s}>{t(s)}</option>)}
                </Select>
              </Field>
              <Field label={t('مُسندة إلى')}>
          ...
          <div className="space-y-2 pt-3 border-t border-slate-100">
            <p className="text-xs font-bold text-slate-500">{t('المحادثة')}</p>
            {data.replies.length === 0 ? (
              <p className="text-sm text-slate-400 text-center py-4">{t('لا توجد ردود بعد.')}</p>
            ) : data.replies.map((r) => (
              <div key={r.id} className="flex items-start gap-2.5">
                ...
                <div className="flex-1 min-w-0 rounded-xl bg-slate-50 px-3 py-2">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-xs font-bold text-slate-600">{r.author_name || t('مستخدم')}</span>
          ...
          {data.status !== 'مغلقة' && (
            <form onSubmit={(e) => { e.preventDefault(); if (reply.trim()) sendReply.mutate() }} className="flex gap-2 pt-2">
              <Input value={reply} onChange={(e) => setReply(e.target.value)} placeholder={t('اكتب رداً...')} className="flex-1" />
              <Button type="submit" loading={sendReply.isLoading}><MessageSquare className="w-4 h-4" /> {t('رد')}</Button>
            </form>
          )}
        </div>
      )}
    ...
    <div className="space-y-6">
      {canManage && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard icon={Inbox} label={t('الإجمالي')} value={s.total ?? 0} tone="blue" />
          <StatCard icon={LifeBuoy} label={t('مفتوحة')} value={s.open ?? 0} tone="amber" />
          <StatCard icon={AlertTriangle} label={t('عاجلة')} value={s.urgent ?? 0} tone="rose" />
          <StatCard icon={UserCheck} label={t('غير مُسندة')} value={s.unassigned ?? 0} tone="violet" />
        </div>
      )}

      <div className="flex justify-end"><Button onClick={() => setShowForm(true)}><Plus className="w-5 h-5" /> {t('تذكرة جديدة')}</Button></div>

      {tickets.length === 0 ? (
        <div className="card"><EmptyState icon={LifeBuoy} title={t('لا توجد تذاكر دعم')} description={t('قدّم تذكرة لأي استفسار أو مشكلة تخص عملك.')} /></div>
      ) : (
        <div className="space-y-3">
          {tickets.map((tk) => (
            <button key={tk.id} onClick={() => setTicketId(tk.id)} className="card w-full text-right flex flex-col sm:flex-row sm:items-center gap-3 hover:shadow-sm transition-shadow">
              <div className="flex items-center gap-3 flex-1 min-w-0">
                {canManage && <Avatar name={tk.full_name} src={tk.profile_picture} size="md" />}
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="font-bold text-slate-800 truncate">{tk.subject}</p>
                    <span className="badge bg-slate-100 text-slate-600">{t(tk.category)}</span>
                    <span className={`badge ${PRIORITY_TONE[tk.priority]}`}>{t(tk.priority)}</span>
                  </div>
                  <p className="text-xs text-slate-400 mt-1">
                    {canManage && tk.full_name ? `${tk.full_name} · ` : ''}{formatDateTime(tk.created_at)}
                    {tk.assigned_to_name ? <> · {t('مُسندة إلى {{name}}', { name: tk.assigned_to_name })}</> : ''}
                    {tk.replies_count > 0 ? <> · {t('{{count}} رد', { count: tk.replies_count })}</> : ''}
                  </p>
                </div>
              </div>
