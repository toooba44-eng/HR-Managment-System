...
function Form({ open, onClose }) {
  ...
  const m = useMutation((d) => grievancesApi.create({ ...d, employee_id: Number(d.employee_id) }), {
    onSuccess: () => { toast.success(t('تم التسجيل')); qc.invalidateQueries('grievances'); onClose() },
    onError: (e) => toast.error(e.response?.data?.error || t('فشلت العملية')),
  })
  const set = (k) => (e) => setForm((f) => ({ ...f, [k]: e.target.value }))
  return (
    <Modal open={open} onClose={onClose} title={t('تسجيل مخالفة/شكوى')}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <Field label={t('النوع')}>
            <Select value={form.type} onChange={set('type')}>
              <option value="شكوى">{t('شكوى')}</option>
              <option value="مخالفة">{t('مخالفة')}</option>
            </Select>
          </Field>
          <Field label={t('الخطورة')}>
            <Select value={form.severity} onChange={set('severity')}>
              <option value="منخفضة">{t('منخفضة')}</option>
              <option value="متوسطة">{t('متوسطة')}</option>
              <option value="عالية">{t('عالية')}</option>
            </Select>
          </Field>
        </div>
        <Field label={t('التصنيف')}><Input value={form.category} onChange={set('category')} placeholder={t('الالتزام بالدوام، بيئة العمل...')} /></Field>
...
function InvestigationModal({ grievance, onClose }) {
  ...
  return (
    <Modal open onClose={onClose} title={t('سجل التحقيق — {{name}}', { name: grievance.full_name })}>
      <div className="space-y-4">
        <p className="text-xs text-slate-400 flex items-center gap-1.5"><Lock className="w-3.5 h-3.5" /> {t('سرّي — مرئي لفريق الموارد البشرية فقط')}</p>
        {isLoading ? <Spinner /> : notes.length === 0 ? (
          <p className="text-sm text-slate-400 text-center py-4">{t('لا توجد ملاحظات بعد.')}</p>
        ) : (
              ...
              <div key={n.id} className="flex items-start gap-2.5">
                ...
                <div className="flex-1 min-w-0 rounded-xl bg-slate-50 px-3 py-2">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-xs font-bold text-slate-600">{n.author_name || t('مستخدم')}</span>
        ...
        <form onSubmit={(e) => { e.preventDefault(); if (note.trim()) addNote.mutate() }} className="flex gap-2 pt-2 border-t border-slate-100">
          <Input value={note} onChange={(e) => setNote(e.target.value)} placeholder={t('أضف ملاحظة تحقيق...')} className="flex-1" />
          <Button type="submit" loading={addNote.isLoading}>{t('إضافة')}</Button>
        </form>
      </div>
    </Modal>
  )
...
export default function Grievances() {
  ...
  const upd = useMutation(({ id, ...rest }) => grievancesApi.update(id, rest), {
    onSuccess: () => { toast.success(t('تم التحديث')); qc.invalidateQueries('grievances') },
    onError: () => toast.error(t('فشل التحديث')),
  })
  const del = useMutation((id) => grievancesApi.remove(id), {
    onSuccess: () => { toast.success(t('تم الحذف')); qc.invalidateQueries('grievances') },
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-end"><Button onClick={() => setShowForm(true)}><Plus className="w-5 h-5" /> {t('تسجيل جديد')}</Button></div>
      {items.length === 0 ? (
        <div className="card"><EmptyState icon={AlertTriangle} title={t('لا توجد مخالفات أو شكاوى')} /></div>
      ) : (
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="badge bg-slate-100 text-slate-600">{t(g.type)}</span>
                      <span className={`badge ${SEV_TONE[g.severity]} inline-flex items-center gap-1`}><Flag className="w-3 h-3" /> {t(g.severity)}</span>
                      <Badge status={g.status}>{t(g.status)}</Badge>
                    </div>
                    <p className="text-xs text-slate-400 mt-0.5">
                      {g.category} · {formatDate(g.created_at)}
                      {g.assigned_to_name ? <> · {t('المحقق: {{name}}', { name: g.assigned_to_name })}</> : ''}
                    </p>
                    {g.description && <p className="text-sm text-slate-600 mt-1">{g.description}</p>}
                    {g.action && <p className="text-xs text-emerald-600 mt-1">{t('الإجراء: {{action}}', { action: g.action })}</p>}
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <button onClick={() => setInvestigating(g)} className={`text-xs px-2.5 py-1.5 rounded-lg border flex items-center gap-1.5 ${g.notes_count > 0 ? 'border-violet-200 bg-violet-50 text-violet-600' : 'border-slate-200 text-slate-500 hover:bg-slate-50'}`}>
                    <Lock className="w-3.5 h-3.5" /> {t('التحقيق')} {g.notes_count > 0 ? `(${g.notes_count})` : ''}
                  </button>
                  <Select value={g.assigned_to || ''} onChange={(e) => upd.mutate({ id: g.id, assigned_to: e.target.value ? Number(e.target.value) : null })} className="text-xs py-1.5 px-2 w-32">
                    <option value="">{t('— محقق —')}</option>
                    {(emps?.employees || []).map((e) => <option key={e.id} value={e.id}>{e.full_name}</option>)}
                  </Select>
                  <Select value={g.status} onChange={(e) => upd.mutate({ id: g.id, status: e.target.value })} className="text-xs py-1.5 px-2 w-32">
                    {STATUSES.map((s) => <option key={s} value={s}>{t(s)}</option>)}
                  </Select>
                  <button onClick={() => window.confirm(t('حذف السجل؟')) && del.mutate(g.id)} className="text-slate-300 hover:text-rose-500"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
            </div>
          ))}
