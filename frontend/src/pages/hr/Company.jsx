...
function ProfileForm({ open, onClose, profile }) {
  ...
  const m = useMutation((d) => companyApi.updateProfile(d), {
    onSuccess: () => { toast.success('تم تحديث بيانات المؤسسة'); qc.invalidateQueries('company'); onClose() },
    onError: (e) => toast.error(e.response?.data?.error || 'فشلت العملية'),
  })
  const set = (k) => (e) => setForm((f) => ({ ...f, [k]: e.target.value }))
  return (
    <Modal open={open} onClose={onClose} title="تعديل بيانات المؤسسة" size="lg">
      <form onSubmit={(e) => { e.preventDefault(); m.mutate(form) }} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <Field label="اسم المؤسسة" required><Input value={form.name || ''} onChange={set('name')} required /></Field>
          <Field label="الاسم القانوني"><Input value={form.legal_name || ''} onChange={set('legal_name')} /></Field>
          <Field label="السجل التجاري"><Input value={form.cr_number || ''} onChange={set('cr_number')} /></Field>
          <Field label="الرقم الضريبي"><Input value={form.tax_number || ''} onChange={set('tax_number')} /></Field>
          <Field label="القطاع"><Input value={form.industry || ''} onChange={set('industry')} /></Field>
          <Field label="حجم المؤسسة"><Input value={form.size || ''} onChange={set('size')} /></Field>
          <Field label="سنة التأسيس"><Input type="number" value={form.founded_year || ''} onChange={set('founded_year')} /></Field>
          ...
          <Field label="الموقع الإلكتروني"><Input value={form.website || ''} onChange={set('website')} /></Field>
          <Field label="المدينة"><Input value={form.city || ''} onChange={set('city')} /></Field>
          <Field label="الدولة"><Input value={form.country || ''} onChange={set('country')} /></Field>
        </div>
        <Field label="العنوان"><Input value={form.address || ''} onChange={set('address')} /></Field>
        <Field label="نبذة"><Input value={form.about || ''} onChange={set('about')} /></Field>
...
function BranchForm({ open, onClose, editing }) {
  ...
  const m = useMutation(
    ...
    {
      onSuccess: () => { toast.success(editing ? 'تم التحديث' : 'تمت الإضافة'); qc.invalidateQueries('company'); onClose() },
      onError: (e) => toast.error(e.response?.data?.error || 'فشلت العملية'),
    },
  )
  const set = (k) => (e) => setForm((f) => ({ ...f, [k]: e.target.value }))
  return (
    <Modal open={open} onClose={onClose} title={editing ? 'تعديل فرع' : 'فرع جديد'}>
      <form onSubmit={(e) => { e.preventDefault(); m.mutate(form) }} className="space-y-4">
        <Field label="اسم الفرع" required><Input value={form.name} onChange={set('name')} required /></Field>
        <div className="grid grid-cols-2 gap-4">
          <Field label="المدينة"><Input value={form.city || ''} onChange={set('city')} /></Field>
          <Field label="الهاتف"><Input value={form.phone || ''} onChange={set('phone')} /></Field>
        </div>
        ...
        {editing && <Field label="الحالة"><Select value={form.status} onChange={set('status')}><option>نشط</option><option>مغلق</option></Select></Field>}
        <div className="flex justify-end gap-3 pt-2">
          <Button type="button" variant="secondary" onClick={onClose}>إلغاء</Button>
          <Button type="submit" loading={m.isLoading}>حفظ</Button>
        </div>
...
export default function Company() {
  ...
  const del = useMutation((id) => companyApi.removeBranch(id), {
    onSuccess: () => { toast.success('تم الحذف'); qc.invalidateQueries('company') },
    ...
    <div className="space-y-6">
      ...
      <div className="card">
        <div className="flex items-start justify-between gap-4">
          ...
          {canManage && <Button variant="secondary" onClick={() => setEditProfile(true)}><Pencil className="w-4 h-4" /> تعديل</Button>}
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-x-8 mt-4 pt-4 border-t border-slate-100">
          <div>
            <InfoRow icon={Hash} label="السجل التجاري" value={p.cr_number} />
            <InfoRow icon={Hash} label="الرقم الضريبي" value={p.tax_number} />
            <InfoRow icon={Users} label="حجم المؤسسة" value={p.size} />
            <InfoRow icon={Calendar} label="سنة التأسيس" value={p.founded_year} />
          </div>
          <div>
            <InfoRow icon={Phone} label="الهاتف" value={p.phone} />
            <InfoRow icon={Mail} label="البريد" value={p.email} />
            <InfoRow icon={Globe} label="الموقع" value={p.website} />
            <InfoRow icon={MapPin} label="العنوان" value={[p.address, p.city, p.country].filter(Boolean).join('، ')} />
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <h3 className="font-bold text-slate-800 flex items-center gap-2"><Briefcase className="w-5 h-5 text-slate-400" /> الفروع ({branches.length})</h3>
        {canManage && <Button onClick={openNewBranch}><Plus className="w-5 h-5" /> فرع جديد</Button>}
      </div>

      {branches.length === 0 ? (
        <div className="card"><EmptyState icon={Briefcase} title="لا توجد فروع" /></div>
      ) : (
            ...
            <div key={b.id} className={`card ${b.status === 'مغلق' ? 'opacity-60' : ''}`}>
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  {b.is_headquarters ? <span className="badge bg-amber-50 text-amber-700 flex items-center gap-1"><Star className="w-3 h-3" /> رئيسي</span> : null}
                </div>
                {canManage && (
                  <div className="flex gap-1">
                    ...
                    <button onClick={() => window.confirm('حذف الفرع؟') && del.mutate(b.id)} className="w-7 h-7 rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-300 hover:text-rose-500"><Trash2 className="w-4 h-4" /></button>
                  </div>
                )}
              </div>
              ...
              {b.manager_name && (
                <div className="flex items-center gap-2 mt-3 pt-3 border-t border-slate-50">
                  ...
                  <div><p className="text-xs font-medium text-slate-700">{b.manager_name}</p><p className="text-[10px] text-slate-400">مدير الفرع</p></div>
                  <div className="mr-auto"><Badge status={b.status} /></div>
                </div>
              )}
            </div>
          ))}
