...
function Form({ open, onClose, editing }) {
  ...
  const m = useMutation(
    ...
    {
      onSuccess: () => { toast.success(editing ? 'تم التحديث' : 'تمت الإضافة'); qc.invalidateQueries('succession'); onClose() },
  ...
  return (
    <Modal open={open} onClose={onClose} title={editing ? 'تعديل خطة التعاقب' : 'خطة تعاقب جديدة'}>
      <form onSubmit={(e) => { e.preventDefault(); m.mutate(form) }} className="space-y-4">
        <Field label="المنصب الحيوي" required><Input value={form.position_title} onChange={set('position_title')} required /></Field>
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="شاغل المنصب الحالي">
            <Select value={form.incumbent_id || ''} onChange={set('incumbent_id')}>
              <option value="">—</option>
              {empOptions.map((e) => <option key={e.id} value={e.id}>{e.full_name}</option>)}
            </Select>
          </Field>
          <Field label="المرشح للخلافة">
            <Select value={form.successor_id || ''} onChange={set('successor_id')}>
              <option value="">لا يوجد</option>
              {empOptions.map((e) => <option key={e.id} value={e.id}>{e.full_name}</option>)}
            </Select>
          </Field>
          <Field label="الجاهزية"><Select value={form.readiness} onChange={set('readiness')}>{READINESS.map((r) => <option key={r}>{r}</option>)}</Select></Field>
          <Field label="مستوى المخاطر"><Select value={form.risk_level} onChange={set('risk_level')}>{RISKS.map((r) => <option key={r}>{r}</option>)}</Select></Field>
          <Field label="تقييم الإمكانات"><Select value={form.potential} onChange={set('potential')}>{POTENTIALS.map((r) => <option key={r}>{r}</option>)}</Select></Field>
        </div>
        <Field label="ملاحظات"><Input value={form.notes || ''} onChange={set('notes')} /></Field>
        {editing && <Field label="الحالة"><Select value={form.status} onChange={set('status')}><option>نشط</option><option>مؤرشف</option></Select></Field>}
...
export default function Talent() {
  ...
  const del = useMutation((id) => successionApi.remove(id), {
    onSuccess: () => { toast.success('تم الحذف'); qc.invalidateQueries('succession') },
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={Network} label="مناصب حيوية" value={s.count ?? 0} tone="blue" />
        <StatCard icon={AlertTriangle} label="مخاطر مرتفعة" value={s.atRisk ?? 0} tone="rose" />
        <StatCard icon={CheckCircle2} label="جاهزون الآن" value={s.readyNow ?? 0} tone="green" />
        <StatCard icon={UserX} label="بدون خليفة" value={s.noSuccessor ?? 0} tone="amber" />
      </div>

      {canManage && <div className="flex justify-end"><Button onClick={openNew}><Plus className="w-5 h-5" /> خطة تعاقب</Button></div>}

      {items.length === 0 ? (
        <div className="card"><EmptyState icon={Star} title="لا توجد خطط تعاقب" description="خطط التعاقب الوظيفي متاحة للإدارة فقط" /></div>
      ) : (
            ...
            <div key={r.id} className={`card ${r.status === 'مؤرشف' ? 'opacity-60' : ''}`}>
              <div className="flex items-start justify-between gap-3">
                ...
                {canManage && (
                  <div className="flex gap-1">
                    ...
                    <button onClick={() => window.confirm('حذف الخطة؟') && del.mutate(r.id)} className="w-7 h-7 rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-300 hover:text-rose-500"><Trash2 className="w-4 h-4" /></button>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center gap-3 mt-4">
                <div className="flex-1 min-w-0 text-center">
                  ...
                  <p className="text-[10px] text-slate-400">الشاغل الحالي</p>
                </div>
                ...
                <div className="flex-1 min-w-0 text-center">
                  {r.successor_name ? (
                    <>
                      ...
                      <p className="text-[10px] text-slate-400">المرشح للخلافة</p>
                    </>
                  ) : (
                    <>
                      ...
                      <p className="text-xs font-medium text-rose-500 mt-1">لا يوجد خليفة</p>
                    </>
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                <Chip label={`الجاهزية: ${r.readiness}`} tone={READY_TONE[r.readiness]} />
                <Chip label={`المخاطر: ${r.risk_level}`} tone={RISK_TONE[r.risk_level]} />
                <Chip label={r.potential} tone={POT_TONE[r.potential]} />
              </div>
