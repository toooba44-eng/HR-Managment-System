...
function VarianceBadge({ variance, hasPlan }) {
  if (!hasPlan) return <span className="badge bg-slate-100 text-slate-400">بلا خطة</span>
  if (variance === 0) return <span className="badge bg-emerald-50 text-emerald-600">مطابق للخطة</span>
  if (variance > 0) return <span className="badge bg-blue-50 text-blue-600 inline-flex items-center gap-1"><TrendingUp className="w-3 h-3" /> فائض {variance}</span>
  return <span className="badge bg-amber-50 text-amber-600 inline-flex items-center gap-1"><TrendingDown className="w-3 h-3" /> نقص {Math.abs(variance)}</span>
}
...
function PlanModal({ dept, year, onClose }) {
  ...
  return (
    <Modal open onClose={onClose} title={`خطة القوى العاملة — ${dept.department_name} (${year})`}>
      <form onSubmit={(e) => { e.preventDefault(); m.mutate() }} className="space-y-4">
        <div className="flex items-center justify-between rounded-xl bg-slate-50 border border-slate-100 px-4 py-3 text-sm">
          <span className="text-slate-500">العدد الفعلي الحالي</span>
          <span className="font-bold text-slate-800">{dept.actual_headcount}</span>
        </div>
        <Field label="العدد المخطط له" required><Input type="number" min="0" value={form.planned_headcount} onChange={set('planned_headcount')} required /></Field>
        <Field label="الميزانية السنوية (ر.س)"><Input type="number" min="0" step="1000" value={form.budget} onChange={set('budget')} /></Field>
        <Field label="ملاحظات"><Textarea value={form.notes} onChange={set('notes')} rows={3} /></Field>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <h2 className="font-bold text-slate-800">تخطيط القوى العاملة</h2>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={Users} label="مخطط / فعلي" value={`${s.planned ?? 0} / ${s.actual ?? 0}`} tone="blue" />
        <StatCard icon={Briefcase} label="شواغر مفتوحة" value={s.open_positions ?? 0} tone="violet" />
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="text-slate-400 text-xs">
              ...
              <th className="p-2 font-medium text-center">المخطط</th>
              <th className="p-2 font-medium text-center">الفعلي</th>
              <th className="p-2 font-medium text-center">الشواغر المفتوحة</th>
              <th className="p-2 font-medium text-center">الميزانية</th>
