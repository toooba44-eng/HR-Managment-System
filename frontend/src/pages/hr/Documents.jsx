...
function Form({ open, onClose, editing }) {
  ...
  const m = useMutation(
    ...
    {
      onSuccess: () => { toast.success(editing ? 'تم التحديث' : 'تم التسجيل'); qc.invalidateQueries('documents-register'); onClose() },
      onError: (e) => toast.error(e.response?.data?.error || 'فشلت العملية'),
    },
  )
  const set = (k) => (e) => setForm((f) => ({ ...f, [k]: e.target.value }))
  return (
    <Modal open={open} onClose={onClose} title={editing ? 'تعديل مستند' : 'تسجيل مستند'}>
      <form onSubmit={(e) => { e.preventDefault(); m.mutate(form) }} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="تاريخ الانتهاء"><Input type="date" value={form.expiry_date || ''} onChange={set('expiry_date')} /></Field>
        </div>
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-3 gap-4">
        ...
        <StatCard icon={FileClock} label="تنتهي قريباً" value={s.expiringSoon ?? 0} tone="amber" />
        <StatCard icon={FileWarning} label="منتهية" value={s.expired ?? 0} tone="rose" />
      </div>
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        {canManage && <Button onClick={openNew}><Plus className="w-5 h-5" /> تسجيل مستند</Button>}
      </div>

      {items.length === 0 ? (
        <div className="card"><EmptyState icon={FileText} title="لا توجد مستندات" /></div>
      ) : (
              ...
              {items.map((r) => (
                <tr key={r.id}>
                  ...
                  <td className="py-3 text-slate-600">
                    {r.expiry_date ? (
                      <span>{formatDate(r.expiry_date)}{r.days_left != null && r.days_left >= 0 && r.days_left <= 30 && <span className="text-amber-600 text-xs mr-1">({r.days_left} يوم)</span>}</span>
                    ) : <span className="text-slate-300">—</span>}
                  </td>
                  <td className="py-3"><Badge status={r.doc_status} /></td>
                  {canManage && (
                    <td className="py-3">
