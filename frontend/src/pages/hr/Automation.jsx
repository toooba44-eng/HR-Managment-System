...
function DetailModal({ wfId, onClose }) {
  ...
  const addCond = useMutation((d) => automationApi.addCondition(wfId, d), { onSuccess: () => { toast.success('تمت الإضافة'); setCond((c) => ({ ...c, value: '' })); invalidate() }, onError: (e) => toast.error(e.response?.data?.error || 'فشل') })
  const delCond = useMutation((id) => automationApi.removeCondition(id), { onSuccess: () => { toast.success('تم الحذف'); invalidate() }, onError: () => toast.error('فشل') })
  const runTest = useMutation(() => automationApi.run(wfId, runEmployeeId), {
    onSuccess: (r) => { toast.success(r.message); invalidate() },
    onError: (e) => toast.error(e.response?.data?.error || 'فشل التنفيذ'),
  })

  const employees = empData?.employees || []

  return (
    <Modal open={!!wfId} onClose={onClose} title="تفاصيل المسار" size="lg">
      {isLoading || !data ? (
        ...
        <div className="space-y-5">
          ...
          {/* Conditions */}
          <div>
            <p className="text-xs font-bold text-slate-500 mb-2 flex items-center gap-1.5"><Filter className="w-3.5 h-3.5" /> الشروط (يجب تحقق الكل)</p>
            <div className="space-y-1.5 mb-2">
              {data.conditions.map((c) => (
                <div key={c.id} className="flex items-center justify-between gap-2 text-sm bg-slate-50 rounded-lg px-3 py-1.5">
                  <span className="text-slate-600">{condFieldLabel(c.field)} {condOpLabel(c.operator)} <b className="text-slate-800">{c.value}</b></span>
                  <button onClick={() => delCond.mutate(c.id)} className="text-slate-300 hover:text-rose-500"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
              ))}
              {data.conditions.length === 0 && <p className="text-xs text-slate-400">بلا شروط — يعمل المسار دائماً عند وقوع الحدث.</p>}
            </div>
            <div className="grid grid-cols-3 gap-2">
              ...
              <div className="flex gap-1">
                <Input value={cond.value} onChange={(e) => setCond((c) => ({ ...c, value: e.target.value }))} placeholder="القيمة" className="!py-1.5 text-xs" />
          ...
          {/* Steps */}
          <div>
            <p className="text-xs font-bold text-slate-500 mb-2">الخطوات (تُنفَّذ بالترتيب عند تحقق الشروط)</p>
          ...
          <div className="rounded-xl bg-slate-50 border border-slate-100 p-4">
            <p className="text-xs font-bold text-slate-500 mb-2 flex items-center gap-1.5"><Play className="w-3.5 h-3.5" /> تشغيل تجريبي على موظف</p>
            ...
            {!data.is_active && <p className="text-[11px] text-amber-600 mt-1.5">المسار غير مفعّل — فعّله أولاً من القائمة.</p>}
          </div>

          {/* Run history */}
          <div>
            <p className="text-xs font-bold text-slate-500 mb-2 flex items-center gap-1.5"><History className="w-3.5 h-3.5" /> سجل التنفيذ</p>
            {data.runs.length === 0 ? (
              <p className="text-xs text-slate-400">لم يُنفَّذ المسار بعد.</p>
            ) : (
              <div className="space-y-1.5 max-h-56 overflow-y-auto">
                {data.runs.map((r) => (
                  <div key={r.id} className="text-xs bg-white border border-slate-100 rounded-lg px-3 py-2">
                    <div className="flex items-center justify-between">
                      <span className={`badge ${r.matched ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}>{r.matched ? 'تحققت الشروط' : 'لم تتحقق الشروط'}</span>
                      <span className="text-slate-400">{formatDateTime(r.created_at)}</span>
                    </div>
                    <p className="text-slate-600 mt-1">{r.employee_name || 'بلا موظف محدد'} · {r.actions_executed} إجراء نُفِّذ من أصل {r.detail.length}</p>
                  </div>
                ))}
              </div>
            )}
          ...
          {workflows.map((w) => (
            <div key={w.id} className={`card ${!w.is_active ? 'opacity-70' : ''}`}>
              ...
              <div className="flex gap-2 mt-3">
                <Button variant="secondary" onClick={() => setDetailId(w.id)} className="flex-1"><Settings2 className="w-4 h-4" /> التفاصيل والتشغيل</Button>
