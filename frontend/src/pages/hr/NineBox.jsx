...
function ReviewModal({ emp, onClose, onShowHistory }) {
  ...
  const save = useMutation(() => talentGridApi.set(emp.id, { performance: perf, potential: pot, notes }), {
    onSuccess: () => { toast.success('تم حفظ التقييم'); qc.invalidateQueries('talent-grid'); onClose() },
    onError: (e) => toast.error(e.response?.data?.error || 'فشل'),
  })
  const clear = useMutation(() => talentGridApi.clear(emp.id), {
    onSuccess: () => { toast.success('تم المسح'); qc.invalidateQueries('talent-grid'); onClose() }, onError: () => toast.error('فشل'),
  })
  const Row = ({ label, value, onChange }) => (
    <div>
      ...
      <div className="flex gap-2">
        {LEVELS.map((l) => (
          <button key={l.v} type="button" onClick={() => onChange(l.v)} className={`flex-1 py-2 rounded-xl text-sm border transition-colors ${value === l.v ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}>{l.label}</button>
        ))}
      </div>
    </div>
  )
  return (
    <Modal open onClose={onClose} title={`تقييم ${emp.full_name}`}>
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={Users} label="مُقيَّمون" value={`${s.reviewed ?? 0}/${s.total ?? 0}`} tone="blue" />
        <StatCard icon={Star} label="نجوم" value={s.stars ?? 0} tone="green" />
        <StatCard icon={AlertTriangle} label="مخاطرة" value={s.risks ?? 0} tone="rose" />
        <StatCard icon={CheckCircle2} label="بانتظار التقييم" value={s.unreviewed ?? 0} tone="amber" />
      </div>

      {unreviewed.length > 0 && (
        <div className="card">
          <h3 className="font-bold text-slate-800 mb-3">بانتظار التقييم</h3>
      ...
      <div className="card overflow-x-auto">
        <div className="flex items-center gap-2 mb-3"><Grid3x3 className="w-5 h-5 text-slate-400" /><h3 className="font-bold text-slate-800">مصفوفة المواهب 9-Box</h3></div>
        <div className="flex gap-2 min-w-[640px]" dir="ltr">
          {/* Potential axis label */}
          <div className="flex items-center"><span className="text-xs font-medium text-slate-400 -rotate-90 whitespace-nowrap">الإمكانات ←</span></div>
          <div className="flex-1">
                  ...
                  {[1, 2, 3].map((perf) => {
                    ...
                    return (
                      <div key={perf} className={`rounded-xl border p-2 min-h-[110px] ${cellTone(perf, pot)}`}>
                        <p className="text-[10px] font-bold text-slate-500 mb-1.5">{boxes[`${perf}-${pot}`]}</p>
            ...
            <div className="text-center mt-2"><span className="text-xs font-medium text-slate-400">الأداء →</span></div>
          </div>
        </div>
      </div>
