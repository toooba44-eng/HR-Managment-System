...
function CompanyForm({ open, onClose, editing }) {
  ...
  const mutation = useMutation(
    ...
    {
      onSuccess: () => { toast.success(editing ? t('تم التحديث') : t('تم إنشاء المؤسسة')); qc.invalidateQueries('companies'); onClose() },
  ...
  return (
    <Modal open={open} onClose={onClose} title={editing ? t('تعديل مؤسسة') : t('إنشاء مؤسسة')}>
      <form onSubmit={(e) => { e.preventDefault(); mutation.mutate(form) }} className="space-y-4">
        <Field label={t('اسم المؤسسة')} required>
          <Input value={form.name} onChange={str('name')} required />
        </Field>
        <Field label={t('البريد الإلكتروني للتواصل')}>
          <Input type="email" value={form.contact_email} onChange={str('contact_email')} />
        </Field>
        <div className="grid grid-cols-2 gap-4">
          <Field label={t('الباقة')}>
            <Select value={form.plan} onChange={str('plan')}>
              {PLANS.map((p) => <option key={p} value={p}>{t(p)}</option>)}
            </Select>
          </Field>
          <Field label={t('الحالة')}>
            <Select value={form.status} onChange={str('status')}>
              <option value="نشطة">{t('نشطة')}</option><option value="معلّقة">{t('معلّقة')}</option>
            </Select>
          </Field>
          <Field label={t('حد المستخدمين')}>
            <Input type="number" min="1" value={form.users_limit} onChange={num('users_limit')} />
          </Field>
          <Field label={t('حد التخزين (GB)')}>
...
export default function Companies() {
  ...
  const toggleMutation = useMutation(
    ...
    { onSuccess: () => { toast.success(t('تم تحديث الحالة')); qc.invalidateQueries('companies') } }
  )
  const removeMutation = useMutation((id) => companiesApi.remove(id), {
    onSuccess: () => { toast.success(t('تم الحذف')); qc.invalidateQueries('companies') },
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={Building} label={t('إجمالي المؤسسات')} value={s.total ?? 0} tone="violet" />
        <StatCard icon={Power} label={t('النشطة')} value={s.active ?? 0} tone="green" />
        <StatCard icon={Power} label={t('المعلّقة')} value={s.suspended ?? 0} tone="amber" />
        <StatCard icon={Users} label={t('الباقات')} value={Object.keys(s.byPlan || {}).length} tone="blue" />
      </div>

      <div className="flex justify-end">
        <Button onClick={() => { setEditing(null); setShowForm(true) }}><Plus className="w-5 h-5" /> {t('مؤسسة جديدة')}</Button>
      </div>

      {companies.length === 0 ? (
        <div className="card"><EmptyState icon={Building} title={t('لا توجد مؤسسات')} /></div>
      ) : (
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge status={c.status}>{t(c.status)}</Badge>
                    </div>
                    <span className="badge bg-violet-50 text-violet-600 mt-1 inline-block">{t('باقة {{plan}}', { plan: t(c.plan) })}</span>
                  </div>
                </div>
                <div className="flex gap-1 shrink-0">
                  <button onClick={() => toggleMutation.mutate(c)} className="w-8 h-8 rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-400 hover:text-amber-600" title={t('تفعيل/تعليق')}><Power className="w-4 h-4" /></button>
                  ...
                  <button onClick={() => window.confirm(t('حذف المؤسسة؟')) && removeMutation.mutate(c.id)} className="w-8 h-8 rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-400 hover:text-rose-500"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
              <div className="mt-4 pt-3 border-t border-slate-100 grid grid-cols-3 gap-2 text-xs">
                <span className="flex items-center gap-1 text-slate-500"><Users className="w-3.5 h-3.5" /> {t('{{count}} مستخدم', { count: c.users_limit })}</span>
