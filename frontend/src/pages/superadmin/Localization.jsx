...
function AddModal({ onClose }) {
  ...
  const m = useMutation((d) => saConfigApi.createLocale(d), {
    onSuccess: () => { toast.success('تمت الإضافة'); qc.invalidateQueries('locales'); onClose() },
    onError: (e) => toast.error(e.response?.data?.error || 'فشلت العملية'),
  })
  const set = (k) => (e) => setForm((f) => ({ ...f, [k]: e.target.value }))
  return (
    <Modal open onClose={onClose} title="إضافة عنصر تعريب">
      <form onSubmit={(e) => { e.preventDefault(); m.mutate(form) }} className="space-y-4">
        <Field label="النوع"><Select value={form.type} onChange={set('type')}>{SECTIONS.map((s) => <option key={s.type}>{s.type}</option>)}</Select></Field>
        <div className="grid grid-cols-2 gap-4">
          <Field label="الاسم" required><Input value={form.name} onChange={set('name')} required /></Field>
          <Field label="الرمز"><Input value={form.code} onChange={set('code')} placeholder="SA / SAR / ar" /></Field>
        </div>
...
export default function Localization() {
  ...
  const upd = useMutation(({ id, ...rest }) => saConfigApi.updateLocale(id, rest), {
    onSuccess: () => qc.invalidateQueries('locales'), onError: () => toast.error('فشل'),
  })
  const del = useMutation((id) => saConfigApi.removeLocale(id), {
    onSuccess: () => { toast.success('تم الحذف'); qc.invalidateQueries('locales') }, onError: () => toast.error('فشل'),
  })
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-end"><Button onClick={() => setAdd(true)}><Plus className="w-5 h-5" /> إضافة</Button></div>

      <div className="grid lg:grid-cols-3 gap-6">
        {SECTIONS.map((sec) => (
          <div key={sec.type} className="card">
            <h3 className="font-bold text-slate-800 flex items-center gap-2 mb-4"><sec.icon className="w-5 h-5 text-slate-400" /> {sec.label}</h3>
            <div className="space-y-1">
              {(grouped[sec.type] || []).map((l) => (
                <div key={l.id} className="flex items-center gap-2 py-2 border-b border-slate-50 last:border-0">
                  ...
                  {!l.is_default && <button onClick={() => upd.mutate({ id: l.id, is_default: true })} className="text-[11px] text-slate-400 hover:text-amber-500" title="تعيين افتراضي"><Star className="w-4 h-4" /></button>}
                  ...
                  {!l.is_default && <button onClick={() => window.confirm('حذف العنصر؟') && del.mutate(l.id)} className="text-slate-300 hover:text-rose-500"><Trash2 className="w-4 h-4" /></button>}
                </div>
              ))}
              {(grouped[sec.type] || []).length === 0 && <p className="text-xs text-slate-400 text-center py-3">لا توجد عناصر</p>}
            </div>
          </div>
        ))}
      </div>
