...
function RespondModal({ survey, onClose }) {
  ...
  const m = useMutation(() => surveysApi.respond(survey.id, { rating, comment }), {
    onSuccess: () => { toast.success(t('شكراً لمشاركتك')); qc.invalidateQueries('surveys'); onClose() },
  ...
  return (
    <Modal open={!!survey} onClose={onClose} title={survey.title}>
      <div className="space-y-4">
        ...
        <div>
          <p className="text-sm font-medium text-slate-700 mb-2">{t('تقييمك')}</p>
          <Stars value={rating} onChange={setRating} />
        </div>
        <Field label={t('ملاحظاتك (اختياري)')}><Input value={comment} onChange={(e) => setComment(e.target.value)} /></Field>
        <div className="flex justify-end gap-3 pt-2">
          <Button variant="secondary" onClick={onClose}>{t('إلغاء')}</Button>
          <Button onClick={() => rating && m.mutate()} loading={m.isLoading} disabled={!rating}>{t('إرسال')}</Button>
        </div>
      </div>
    </Modal>
  )
...
function ResultsModal({ surveyId, onClose }) {
  ...
  return (
    <Modal open={!!surveyId} onClose={onClose} title={t('نتائج الاستطلاع')} size="lg">
      {isLoading || !data ? <div className="py-12"><Spinner /></div> : (
        <div className="space-y-4">
          {data.survey.anonymous ? (
            <p className="text-xs text-slate-400 flex items-center gap-1.5"><EyeOff className="w-3.5 h-3.5" /> {t('استطلاع مجهول — لا تُعرض هوية المشاركين')}</p>
          ) : null}
          <div className="flex items-center gap-6">
            <div className="text-center"><p className="text-3xl font-extrabold text-amber-500">{data.stats.avg_rating ?? '—'}</p><p className="text-xs text-slate-400">{t('متوسط التقييم')}</p></div>
            <div className="text-center"><p className="text-3xl font-extrabold text-slate-800">{data.stats.count}</p><p className="text-xs text-slate-400">{t('عدد المشاركات')}</p></div>
          </div>
          ...
          <div className="space-y-2 pt-2 border-t border-slate-100">
            ...
            {data.responses.length === 0 && <p className="text-sm text-slate-400 text-center py-4">{t('لا توجد مشاركات بعد')}</p>}
          </div>
        </div>
      )}
    </Modal>
...
function CreateModal({ onClose }) {
  ...
  const m = useMutation((d) => surveysApi.create(d), {
    onSuccess: () => { toast.success(t('تم إنشاء الاستطلاع')); qc.invalidateQueries('surveys'); onClose() },
    onError: (e) => toast.error(e.response?.data?.error || t('فشلت العملية')),
  })
  return (
    <Modal open onClose={onClose} title={t('استطلاع جديد')}>
      <form onSubmit={(e) => { e.preventDefault(); m.mutate(form) }} className="space-y-4">
        <Field label={t('عنوان الاستطلاع')} required><Input value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} required /></Field>
        <Field label={t('الوصف')}><Input value={form.description} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))} /></Field>
        <label className="flex items-center gap-2 text-sm text-slate-600">
          ...
          {t('استطلاع مجهول (لا تُعرض هوية المشاركين في النتائج)')}
        </label>
        <div className="flex justify-end gap-3 pt-2">
...
export default function Surveys() {
  ...
  const del = useMutation((id) => surveysApi.remove(id), {
    onSuccess: () => { toast.success(t('تم الحذف')); qc.invalidateQueries('surveys') }, onError: () => toast.error(t('فشل')),
  })
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-3 gap-4">
        <StatCard icon={ClipboardList} label={t('الاستطلاعات')} value={s.total ?? 0} tone="blue" />
        <StatCard icon={CheckCircle2} label={t('نشطة')} value={s.active ?? 0} tone="green" />
        <StatCard icon={MessageSquare} label={t('إجمالي المشاركات')} value={s.responses ?? 0} tone="violet" />
      </div>

      {canManage && <div className="flex justify-end"><Button onClick={() => setShowCreate(true)}><Plus className="w-5 h-5" /> {t('استطلاع جديد')}</Button></div>}

      {items.length === 0 ? (
        <div className="card"><EmptyState icon={ClipboardList} title={t('لا توجد استطلاعات')} /></div>
      ) : (
          ...
          {items.map((sv) => (
            <div key={sv.id} className={`card ${!sv.is_active ? 'opacity-70' : ''}`}>
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {!!sv.anonymous && <span className="badge bg-violet-50 text-violet-600 inline-flex items-center gap-1"><EyeOff className="w-3 h-3" /> {t('مجهول')}</span>}
                  </div>
                  {sv.description && <p className="text-sm text-slate-500 mt-1">{sv.description}</p>}
                </div>
                <span className={`badge ${sv.is_active ? 'badge-success' : 'bg-slate-100 text-slate-500'}`}>{sv.is_active ? t('نشط') : t('مغلق')}</span>
              </div>
              <div className="flex items-center gap-4 mt-3 pt-3 border-t border-slate-50 text-xs text-slate-500">
                <span className="flex items-center gap-1"><MessageSquare className="w-4 h-4 text-slate-300" /> {t('{{count}} مشاركة', { count: sv.responses_count })}</span>
              ...
              <div className="flex gap-2 mt-3">
                {sv.responded ? (
                  <Button variant="secondary" disabled className="flex-1"><CheckCircle2 className="w-4 h-4" /> {t('شاركت')}</Button>
                ) : sv.is_active ? (
                  <Button onClick={() => setRespond(sv)} className="flex-1"><Star className="w-4 h-4" /> {t('شارك')}</Button>
                ) : (
                  <Button variant="secondary" disabled className="flex-1">{t('مغلق')}</Button>
                )}
                ...
                {canManage && <button onClick={() => window.confirm(t('حذف الاستطلاع؟')) && del.mutate(sv.id)} className="w-9 rounded-xl hover:bg-slate-100 flex items-center justify-center text-slate-300 hover:text-rose-500"><Trash2 className="w-4 h-4" /></button>}
              </div>
            </div>
          ))}
        </div>
