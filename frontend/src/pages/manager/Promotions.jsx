...
function CreateModal({ onClose }) {
  ...
  const m = useMutation((d) => promotionsApi.create(d), {
    onSuccess: () => { toast.success(t('تم رفع الطلب')); qc.invalidateQueries('promotions'); onClose() },
    onError: (e) => toast.error(e.response?.data?.error || t('فشلت العملية')),
  })
  const set = (k) => (e) => setForm((f) => ({ ...f, [k]: e.target.value }))
  return (
    <Modal open onClose={onClose} title={t('طلب ترقية / نقل')}>
      <form onSubmit={(e) => { e.preventDefault(); m.mutate(form) }} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <Field label={t('الموظف')} required><Select value={form.employee_id} onChange={set('employee_id')} required><option value="">{t('اختر')}</option>{(emps?.employees || []).map((e) => <option key={e.id} value={e.id}>{e.full_name}</option>)}</Select></Field>
          <Field label={t('النوع')}><Select value={form.type} onChange={set('type')}><option value="ترقية">{t('ترقية')}</option><option value="نقل">{t('نقل')}</option></Select></Field>
        </div>
        {form.type === 'ترقية' ? (
          <Field label={t('المسمى الجديد')} required><Input value={form.new_title} onChange={set('new_title')} required /></Field>
        ) : (
          <Field label={t('القسم الجديد')} required><Select value={form.new_department_id} onChange={set('new_department_id')} required><option value="">{t('اختر')}</option>{(depts || []).map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}</Select></Field>
        )}
        <div className="grid grid-cols-2 gap-4">
...
export default function Promotions() {
  ...
  const setStatus = useMutation(({ id, status }) => promotionsApi.setStatus(id, status), {
    onSuccess: () => { toast.success(t('تم التحديث')); qc.invalidateQueries('promotions') }, onError: () => toast.error(t('فشل')),
  })
  const del = useMutation((id) => promotionsApi.remove(id), {
    onSuccess: () => { toast.success(t('تم الحذف')); qc.invalidateQueries('promotions') }, onError: () => toast.error(t('فشل')),
  })
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard icon={TrendingUp} label={t('ترقيات')} value={s.promotions ?? 0} tone="green" />
        <StatCard icon={Shuffle} label={t('عمليات نقل')} value={s.transfers ?? 0} tone="violet" />
      </div>

      <div className="flex justify-end"><Button onClick={() => setShowCreate(true)}><Plus className="w-5 h-5" /> {t('طلب ترقية/نقل')}</Button></div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {items.map((p) => (
            <div key={p.id} className="card">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className={`badge ${p.type === 'ترقية' ? 'bg-emerald-50 text-emerald-700' : 'bg-violet-50 text-violet-700'}`}>{t(p.type)}</span>
                  </div>
                </div>
                <Badge status={p.status}>{t(p.status, { context: 'promotion' })}</Badge>
              </div>
              ...
              {p.effective_date && <p className="text-xs text-slate-400 mt-2">{t('تاريخ السريان')}: {formatDate(p.effective_date)}</p>}
              {p.justification && <p className="text-sm text-slate-500 mt-2">{p.justification}</p>}
              <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-50">
                <span className="text-xs text-slate-400">{t('مقدّم الطلب: {{name}}', { name: p.requested_by_name || '—' })}</span>
                <div className="flex gap-1">
                  ...
                  <button onClick={() => window.confirm(t('حذف الطلب؟')) && del.mutate(p.id)} className="w-7 h-7 rounded-lg hover:bg-slate-100 flex items-center justify-center text-slate-300 hover:text-rose-500"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
            </div>
          ))}
